...
class SecureAttendApp:
    ...
    def load_registered_users(self):
        ...
        try:
            ...
            for filename in os.listdir(images_path):
                if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                    ...
                    if user_info:
                        ...
                        # Format: regno_name or just name
                        if '_' in user_id:
                            parts = user_id.split('_', 1)  # Split on first underscore
            ...
            self.update_status(f"Loaded {loaded_count} registered users")

            if loaded_count == 0:
                print("⚠ WARNING: No users loaded!")
                print(f"Check if images exist in: {os.path.abspath(images_path)}")
